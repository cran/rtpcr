...
1. Experimental condition columns  
2. Biological replicate information (if applicable)  
3. Target gene efficiency (`E_target`)  
4. Target gene Ct (`Ct_target`)  
5. Reference gene efficiency (`E_ref`)  
6. Reference gene Ct (`Ct_ref`)
...
2. Factor columns follow (including the `time` variable)  
...
```{r eval= T, fig.height = 4, fig.width = 5, fig.align = 'center', fig.cap = "Fold change expression of two different genes. FC tables of any number of genes can be combined and used as input data frame for `twoFACTORplot` function."}
